...
with tf.Session(config=config) as sess:
    ...
    if config.SETP5_IF_RESTORE_GEMINI_MODEL:
        saver.restore(sess, config.MODEL_GEMINI_DIR + os.sep + config.STEP5_GEMINI_MODEL_TO_RESTORE)
    ...
    while iter < max_iter:
        ...
        if iter%snapshot == 0:
            ...
            saver.save(sess, config.MODEL_GEMINI_DIR + os.sep + "gemini-model"+PREFIX+"_"+str(iter)+".ckpt")
...
# 保存模型
    save_path = saver.save(sess, config.MODEL_GEMINI_DIR + os.sep + "gemini-model"+PREFIX+"_final.ckpt")
    print save_path
